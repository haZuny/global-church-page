...
export default async function NewsDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try { entry = await getNewsEntry(slug); } catch { return <div className="subpage"><main id="main-content" className="section"><div className="page-shell"><ContentState title="주보·소식을 불러오지 못했습니다." description="잠시 후 다시 시도해 주세요."/></div></main></div>; }
  if (!entry) notFound();
