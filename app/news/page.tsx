...
export default function NewsPage() {
  return <div className="subpage"><main id="main-content">
    ...
    <section className="notice-archive section" aria-label="주보와 소식 목록"><div className="page-shell"><div className="archive-toolbar reveal"><nav aria-label="소식 분류"><span className="is-active">전체</span><span>주보</span><span>공지</span><span>행사</span></nav><span>2026</span></div>
      ...
      <div className="notice-list reveal" id="bulletin-list"><div className="notice-list__head" aria-hidden="true"><span>날짜</span><span>분류</span><span>제목</span><span/></div>{bulletins.map(([slug, date, category, title, summary]) => <Link href={`/news/${slug}`} key={slug}><time dateTime={slug.startsWith("2026-") && slug !== "2026-lunar-new-year" ? slug : undefined}>{date}</time><span>{category}</span><div><strong>{title}</strong><p>{summary}</p></div><i aria-hidden="true">→</i></Link>)}</div>
