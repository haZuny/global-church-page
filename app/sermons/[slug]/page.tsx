...
export default async function SermonDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let sermon: Awaited<ReturnType<typeof getSermon>>;
  try { sermon = await getSermon(slug); } catch { return <div className="subpage"><main id="main-content" className="section"><div className="page-shell"><ContentState title="설교를 불러오지 못했습니다." description="잠시 후 다시 시도해 주세요."/></div></main></div>; }
  if (!sermon) notFound();
