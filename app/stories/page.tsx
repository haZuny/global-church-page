...
export default function StoriesPage() {
  return <div className="subpage"><main id="main-content">
    ...
    <section className="archive-content section" aria-label="교회 이야기 목록"><div className="page-shell"><div className="archive-toolbar reveal"><nav aria-label="이야기 분류"><span className="is-active">전체</span><span>예배</span><span>공동체</span><span>이웃 섬김</span><span>다음 세대</span></nav><span>PUBLIC RECORDS</span></div>
      <article className="archive-feature reveal"><Link href="/stories/youth-vision-founder" aria-label="청소년부 비전파운더 이야기 보기"><div className="archive-feature__image"><img src={featuredStory.image} alt={featuredStory.alt}/></div><div className="archive-feature__copy"><p><span>{featuredStory.category}</span><time dateTime={featuredStory.dateTime}>{featuredStory.date}</time></p><h2>{featuredStory.title}</h2><div><p>{featuredStory.subtitle}</p><span aria-hidden="true">→</span></div></div></Link></article>
      <div className="archive-grid">{storyRows.map(([slug, story, imageClass]) => <article className="archive-card reveal" key={slug}><Link href={`/stories/${slug}`}><div className={`archive-card__image ${imageClass}`}><img src={story.image} alt={story.alt} loading="lazy"/></div><p><span>{story.category}</span><time dateTime={story.dateTime}>{story.date}</time></p><h2>{story.title}</h2><div>{story.summary}</div></Link></article>)}</div>
