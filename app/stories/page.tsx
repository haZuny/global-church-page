...
export default async function StoriesPage() {
  ...
  return <div className="subpage"><main id="main-content">
    ...
    <section className="archive-content section" aria-label="교회 이야기 목록"><div className="page-shell"><div className="archive-toolbar reveal"><span>PUBLIC RECORDS</span></div>
      <article className="archive-feature reveal"><Link href={`/stories/${featuredStory.id}`} aria-label={`${featuredStory.title} 이야기 보기`}><div className="archive-feature__image"><Image src={featuredStory.image} alt={featuredStory.alt} fill unoptimized sizes="(max-width: 780px) 100vw, 54vw"/></div><div className="archive-feature__copy"><p><time dateTime={featuredStory.dateTime}>{featuredStory.date}</time></p><h2>{featuredStory.title}</h2><div><p>{featuredStory.summary}</p><span aria-hidden="true">→</span></div></div></Link></article>
      <div className="archive-grid">{storyRows.map((story) => <article className="archive-card reveal" key={story.id}><Link href={`/stories/${story.id}`}><div className="archive-card__image"><Image src={story.image} alt={story.alt} fill unoptimized sizes="(max-width: 780px) 100vw, 50vw"/></div><p><time dateTime={story.dateTime}>{story.date}</time></p><h2>{story.title}</h2><div>{story.summary}</div></Link></article>)}</div>
