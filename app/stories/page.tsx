...
export default async function StoriesPage() {
  ...
  return <div className="subpage"><main id="main-content">
    ...
    <section className="archive-content section" aria-label="교회 이야기 목록"><div className="page-shell"><div className="archive-toolbar reveal"><nav aria-label="이야기 분류"><span className="is-active">전체</span><span>예배</span><span>공동체</span><span>이웃 섬김</span><span>다음 세대</span></nav><span>PUBLIC RECORDS</span></div>
      <article className="archive-feature reveal"><Link href={`/stories/${featuredStory.slug}`} aria-label={`${featuredStory.title} 이야기 보기`}><div className="archive-feature__image"><Image src={featuredStory.image} alt={featuredStory.alt} fill unoptimized sizes="(max-width: 780px) 100vw, 54vw"/></div><div className="archive-feature__copy"><p><span>{featuredStory.category}</span><time dateTime={featuredStory.dateTime}>{featuredStory.date}</time></p><h2>{featuredStory.title}</h2><div><p>{featuredStory.subtitle}</p><span aria-hidden="true">→</span></div></div></Link></article>
      <div className="archive-grid">{storyRows.map((story) => <article className="archive-card reveal" key={story.slug}><Link href={`/stories/${story.slug}`}><div className="archive-card__image"><Image src={story.image} alt={story.alt} fill unoptimized sizes="(max-width: 780px) 100vw, 50vw"/></div><p><span>{story.category}</span><time dateTime={story.dateTime}>{story.date}</time></p><h2>{story.title}</h2><div>{story.summary}</div></Link></article>)}</div>
