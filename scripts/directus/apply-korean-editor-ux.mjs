...
const models = {
  ...
  church_ministers: ["교역자", "교회정보에 표시할 교역자를 관리합니다.", { site_settings: "교회정보", name: "이름", role: "역할", description: "소개", photo: "프로필 사진", sort: "노출 순서", status: "게시 상태" }],
  worship_services: ["예배 안내", "방문자가 확인하는 예배 시간과 장소를 관리합니다.", { name: "예배 이름", audience: "대상", weekday: "이전 요일", weekdays: "요일", start_time: "시작 시간", location: "장소", description: "안내 문구", sort: "노출 순서", status: "게시 상태" }],
...
for (const [collection, [label, note, labels]] of Object.entries(models)) {
  ...
  for (const [field, translation] of Object.entries(labels)) {
    ...
    await request(`/fields/${collection}/${field}`, "PATCH", { meta: { ...currentField.meta, ...statusOptions, ...categoryOptions, ...typeOptions, ...weekdayOptions, ...worshipTimeOptions, ...worshipSortOptions, ...dateOptions, ...fileOptions, special: field === "weekdays" ? ["cast-json"] : currentField.meta?.special, interface: interfaceName, options, hidden: legacyPath || field === "slug" || (collection === "stories" && ["subtitle", "category", "cover_image", "cover_alt"].includes(field)) || (collection === "bulletins" && ["document_file", "document_alt"].includes(field)) || (collection === "sermons" && field === "video_url") || (collection === "worship_services" && ["audience", "weekday"].includes(field)) || ["type", "event_starts_at", "event_ends_at", "location"].includes(field), sort: currentField.meta?.sort ?? 1, translations: [{ language: "ko-KR", translation }] } });
  }
}
