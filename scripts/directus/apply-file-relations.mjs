...
const fileFields = await fetch(`${baseUrl}/fields/directus_files`, { headers }).then((response) => response.json()).then((body) => body.data);
...
for (const reverseField of reverseFields.values()) {
  if (fileFields.some((field) => field.field === reverseField)) continue;
  const response = await fetch(`${baseUrl}/fields/directus_files`, { method: "POST", headers, body: JSON.stringify({ field: reverseField, type: "alias", meta: { special: ["o2m"], hidden: true, interface: "list-o2m", readonly: true }, schema: null }) });
  const body = await response.json();
  if (!response.ok) throw new Error(body.errors?.[0]?.message ?? "Reverse file field creation failed.");
}
...
for (const [collection, field] of [["stories", "cover_image"], ["bulletins", "document_file"], ["sermons", "video_file"]]) {
  ...
  const response = await fetch(`${baseUrl}/relations`, { method: "POST", headers, body: JSON.stringify({ collection, field, related_collection: "directus_files", meta: { many_collection: collection, many_field: field, one_collection: "directus_files", one_field: reverseFields.get(`${collection}.${field}`), one_deselect_action: "nullify" }, schema: { table: collection, column: field, foreign_key_table: "directus_files", foreign_key_column: "id", on_delete: "SET NULL", on_update: "NO ACTION" } }) });
  const body = await response.json();
