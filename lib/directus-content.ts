...
export type StoryEntry = { slug: string; category: string; subtitle: string; date: string; dateTime: string; title: string; summary: string; image: string; imageWidth: number; imageHeight: number; alt: string; blocks: StoryBlock[] };
...
export async function getBulletins() {
  ...
  return items.map((item): BulletinEntry => ({ slug: item.slug, title: item.title, date: dateLabel(item.published_at), category: item.category, summary: item.summary, image: item.document_image_url, imageWidth: item.document_image_width, imageHeight: item.document_image_height }));
}

export async function getBulletin(slug: string) {
  return (await getBulletins()).find((bulletin) => bulletin.slug === slug);
}
