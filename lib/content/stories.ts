...
export const stories: Record<string, StoryEntry> = {
  "vision-founder": {
    category: "지유쓰 · 청년부",
    subtitle:
      "삶의 목적과 하나님의 뜻을 함께 묻고, 우리 안에 주신 비전을 발견해 간 3일의 기록",
    date: "2026. 07. 01",
    dateTime: "2026-07-01",
  ...
  "youth-dining-room": {
    ...
    blocks: [
      {
        type: "text",
        text: "청년대학부가 함께 사용할 식당 공간을 손수 꾸몄습니다.",
      },
      {
        type: "text",
  ...
  "mens-cell-presentation": {
    ...
    blocks: [
      ...
      {
        type: "text",
        text: "함께 배우고 응원하며 교제한 이날의 모습을 사진으로 남겼습니다.",
      },
    ],
  },
};
...
export const storyRows = [
  ...
  [
    "youth-dining-room",
    stories["youth-dining-room"],
    "archive-card__image--lobby archive-card__image--thumb",
  ],
  [
    "youth-winter-camp",
    stories["youth-winter-camp"],
    "archive-card__image--detail",
  ],
  [
    "mens-cell-presentation",
    stories["mens-cell-presentation"],
    "archive-card__image--detail",
  ],
] as const;

export const storySlugs = Object.keys(stories).sort(
  (first, second) =>
    new Date(stories[second].dateTime).getTime() -
    new Date(stories[first].dateTime).getTime(),
);

export function getStoryNavigation(slug: string) {
  const currentIndex = storySlugs.indexOf(slug);

  return {
    previousSlug: currentIndex > 0 ? storySlugs[currentIndex - 1] : undefined,
    nextSlug:
      currentIndex >= 0 && currentIndex < storySlugs.length - 1
        ? storySlugs[currentIndex + 1]
        : undefined,
  };
}
