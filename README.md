...
주보와 공지는 별도 최상위 메뉴로 강조하기보다 홈 및 관련 화면에서 보조 콘텐츠로 노출하는 방향을 우선 검토합니다.
...
현재 저장소에는 기획 방향을 확인하기 위한 정적 단일 페이지 데모가 포함되어 있습니다. 별도 설치 없이 `index.html`을 브라우저에서 열거나 간단한 로컬 서버를 실행하면 됩니다.
...
데모의 `온마음교회`, 예배 시간, 주소, 연락처와 게시물은 모두 콘셉트 확인을 위한 예시입니다. 생성된 이미지 또한 최종 운영용 실제 교회 사진으로 교체해야 합니다.
